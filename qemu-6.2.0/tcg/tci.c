...
static const char *str_c(TCGCond c)
{
    ...
    if((unsigned)c > ARRAY_SIZE(cond) || cond[c][0] == 0){
        return cond[0];
    }else{
        return cond[c];
    }
...
int print_insn_tci(bfd_vma addr, disassemble_info *info)
{
    ...
    info->fprintf_func(info->stream, "%016lx  ", addr);
    ...
    case INDEX_op_st_i64:
        ...
        info->fprintf_func(info->stream, "%-12s  %s, %s, 0x%x",
    ...
    do_qemu_ldst:
        ...
        case 3:
            tci_args_rrrm(insn, &r0, &r1, &r2, &oi);
            info->fprintf_func(info->stream, "%-12s  %s, %s, %s, 0x%x",
