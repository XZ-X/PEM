...
#include "linux-user/pem_load_info.h"
...
static void load_elf_image(const char *image_name, int image_fd,
                           ...
                           char bprm_buf[BPRM_BUF_SIZE])
{
    int load_main_exec = strcmp(image_name, exec_path) == 0;
    printf("Load %s\n", image_name);
    // if(load_main_exec){
    //     printf("Exec has been loaded\n");
    //     load_info.loaded = 1;
    // }
     ...
     * from the executable.
     */
    load_addr = target_mmap(loaddr, hiaddr - loaddr, PROT_READ,
    ...
    if(load_main_exec){
        load_info.load_bias = load_bias;
        load_info.load_lo = load_addr;
        load_info.load_hi = load_addr + elf_hiaddr - loaddr;
        load_info.entry = ehdr->e_entry + load_bias;        
    }
    ...
    if(load_main_exec){
        load_info.exec_start = info->start_code;
        load_info.exec_end = info->end_code;
        load_info.data_start = info->start_data;
        load_info.data_end = info->end_data;
    }
