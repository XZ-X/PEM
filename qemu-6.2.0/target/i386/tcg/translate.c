...
static target_ulong disas_insn(DisasContext *s, CPUState *cpu)
{
 ...
 reswitch:
    ...
    if(0!=has_string_indicator){
        gen_helper_str_indicator();
    }
    return s->pc;
 illegal_op:
    gen_illegal_opcode(s);
...
void HELPER(call_indicator)(){    
    if(load_info.loaded){
        assert(false);
    }    
}

void HELPER(ret_indicator)(){
    if(load_info.loaded){
        assert(false);
    }
}

void HELPER(str_indicator)(){
    if(load_info.loaded){
        assert(false);
    }
}