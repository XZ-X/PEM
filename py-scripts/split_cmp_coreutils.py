...
def parse_args(parser=None, add_only=False):
    ...
    parser.add_argument(
        "--name-map",
        type=str,
        default="qemu-tci-linux-user/benchmarks/eval-dataset/eval-coreutils/name_map.clang12.pkl",
        help="Name map file",
    )
    parser.add_argument("--out-dir", type=str, default="qemu-tci-linux-user/benchmarks/cmp-ret/eval-dataset/eval-coreutils/clang12O2.vs.clang12O3", help="Ratio")    
    if add_only:
        return parser
    else:
...
if __name__ == "__main__":
    ...
    coreutils_name_map = pickle.load(open(args.name_map, "rb"))

    for util_name, func_names in coreutils_name_map.items():
        ret = {}
        current_has_answer = func_names & has_answer
        src_addrs = [name2addr_src[x] for x in current_has_answer]
        tgt_addrs = [name2addr_tgt[x] for x in current_has_answer]
        for src_addr in src_addrs:
            for tgt_addr in tgt_addrs:
                if (src_addr, tgt_addr) in cmp_dict:
                    ret[(src_addr, tgt_addr)] = cmp_dict[src_addr, tgt_addr]
        fout = open(os.path.join(args.out_dir, util_name+".cmp.pkl"), "wb")
    ...
    # results = {}
    # for name in coreutils_name_map.keys():
    #     results[name] = []
